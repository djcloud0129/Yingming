...
桌宠支持主动陪伴模式，可以在 `连接` 窗口里选择：`安静陪着` 不主动插话，`正常主动` 会在闲置一段时间后轻提醒，`稍微黏一点` 会更快一点关心你。主动提醒会避开正在输入、等待回复、收起状态和打开设置/记忆/画像窗口的时候。

如果樱茗上一句正在等你回答，比如问了“你看过吗？”或说“等你回来告诉我感受”，主动陪伴会暂停，不会另开新话题。右键菜单里也有 `回到刚才话题` 和 `等我回答`，你可以手动把她拉回上一段对话，或者让她安静等你。

樱茗还会从最近聊天中推断短期话题，例如电影、项目、情绪或记忆整理，并标记为进行中、等你回答、暂停或已收束。短期话题只用于当前会话节奏，不会写入长期记忆；主动陪伴会在话题未收束时保持安静，避免你还没回她，她就自己开启新话题。

聊天时，樱茗会识别临时对话状态，例如 `项目模式`、`陪伴模式`、`记忆整理`、`深夜收尾`、`少说一点`。这个状态只影响当下回复和桌宠右上角的小状态标签，不会自动写入长期记忆。

桌宠会把临时状态映射成轻量 mood，例如 `专注`、`温柔`、`思考`、`收尾`、`整理`、`等你`。头像边框、气泡底色和右上角状态会随 mood 变化；切换 mood 时头像会轻微 pulse 一下，作为第一版表情与动作反馈。

头像区域现在还有一层轻量舞台反馈：樱茗会显示当前表情和动作，例如 `认真 / 靠近看`、`在想 / 整理思路`、`等你 / 安静听着`。头像会随状态轻微呼吸、点头、摇晃或待命；你在输入框打字时，她会切到“在听”的状态。

樱茗内部现在有一层轻量事件总线和行为协议。聊天、回复、mood 变化、话题变化、身体状态、相关记忆检索、待确认记忆和头像动作都会生成统一事件，例如 `user.message`、`assistant.reply`、`mood.changed`、`topic.updated`、`body.state`、`memory.retrieved`、`memory.suggested`、`pet.action`。这不会改变当前使用方式，但为后续接入语音、Live2D、屏幕感知和更完整的动作状态机打好地基。
...
聊天时，樱茗会先用本地轻量检索从已确认长期记忆里挑出当前最相关的几条，再交给模型回答；基础信息和相处边界会常驻，待确认记忆不会被当作事实使用。这样她会更像是在“想起相关的事”，而不是每次把全部记忆硬塞进上下文。
